...
import { Table, Button, Space, Tag, Select, DatePicker, message, Popconfirm, Card, Modal, Form, InputNumber, Input } from 'antd';
...
function Purchases() {
  ...
  const columns = [
    { title: '入库单号', dataIndex: 'invoiceNo', width: 150 },
    { title: '供应商', dataIndex: 'supplierName' },
    { title: '入库金额', dataIndex: 'amount', render: v => `¥${v.toLocaleString()}` },
    { title: '已付金额', dataIndex: 'paidAmount', render: v => `¥${v.toLocaleString()}` },
    { title: '未付金额', render: (_, r) => `¥${(r.amount - r.paidAmount).toLocaleString()}` },
    ...
    { title: '操作', width: 120, render: (_, r) => (<Space><Button type="link" size="small" icon={<EditOutlined />} onClick={() => { setEditing(r); form.setFieldsValue({ ...r, purchaseDate: dayjs(r.purchaseDate) }); setModalVisible(true); }}>编辑</Button><Popconfirm title="确定删除？" onConfirm={() => handleDelete(r.id)}><Button type="link" size="small" danger icon={<DeleteOutlined />}>删除</Button></Popconfirm></Space>) }
  ];
