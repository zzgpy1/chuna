...
import { Card, Row, Col, Select, DatePicker, Button, Table, Statistic, Space, Radio, Divider, message, Typography } from 'antd';
import { SearchOutlined, FileExcelOutlined, DollarOutlined, ShoppingOutlined, CalendarOutlined } from '@ant-design/icons';
import { db } from '../db';
...
function Reports() {
  ...
  const getDateRange = () => {
    ...
    if (queryType === 'day') {
      ...
      return { start: dayjs().startOf('year').toDate(), end: dayjs().endOf('year').toDate() };
    } else if (queryType === 'custom' && customRange) {
  ...
  const handleQuery = async () => {
    ...
    try {
      ...
      if (selectedSupplier) {
        paymentsQuery = paymentsQuery.and(p => p.supplierId === selectedSupplier);
        purchasesQuery = purchasesQuery.and(p => p.supplierId === selectedSupplier);
      }
      ...
      const unpaidTotal = purchasesTotal - totalPaidForPurchases;
      
      const supplierMap = new Map(suppliers.map(s => [s.id, s.name]));
      const enrichedPayments = payments.map(p => ({ ...p, supplierName: supplierMap.get(p.supplierId) || '未知' }));
      
      setResult({ payments: enrichedPayments, total: totalPaid, purchasesTotal, unpaidTotal: Math.max(0, unpaidTotal) });
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    ...
    { title: '确认时间', dataIndex: 'confirmDate', render: v => v ? dayjs(v).format('YYYY-MM-DD') : '-' },
  ];
    ...
    <div>
      ...
      <Card style={{ marginBottom: 24 }}>
        <Row gutter={[16, 16]} align="middle">
          <Col span={6}><Select placeholder="选择供应商 (可选)" allowClear style={{ width: '100%' }} onChange={setSelectedSupplier}><Option value={null}>全部供应商</Option>{suppliers.map(s => <Option key={s.id} value={s.id}>{s.name}</Option>)}</Select></Col>
      ...
      {result.payments.length > 0 && (
        <>
          <Row gutter={16} style={{ marginBottom: 24 }}>
            <Col span={8}><Card><Statistic title="付款总额" value={result.total} prefix="¥" precision={2} valueStyle={{ color: '#52c41a' }} /></Card></Col>
            <Col span={8}><Card><Statistic title="入库总额" value={result.purchasesTotal} prefix="¥" precision={2} valueStyle={{ color: '#1677ff' }} /></Card></Col>
            <Col span={8}><Card><Statistic title="未付金额" value={result.unpaidTotal} prefix="¥" precision={2} valueStyle={{ color: '#faad14' }} /></Card></Col>
          </Row>
          <Table columns={columns} dataSource={result.payments} rowKey="id" pagination={{ pageSize: 10 }} />
        </>
      )}
