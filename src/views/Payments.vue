<template>
  <div class="payments">
    ...
    <el-table :data="filteredPayments" stripe v-loading="loading" style="width: 100%" :max-height="600">
      ...
      <el-table-column prop="payment_type" label="付款类型" width="100">
        <template #default="{ row }">
          <el-tag :type="row.payment_type === 'prepay' ? 'info' : 'success'">{{ row.payment_type === 'prepay' ? '预付款' : '后付款' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="180" fixed="right">
        <template #default="{ row }">
          <el-button link type="primary" @click="openConfirmDialog(row)" v-if="row.status === 'unpaid'">确认打款</el-button>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="550px">
      <el-form :model="paymentForm" :rules="paymentRules" ref="paymentFormRef" label-width="100px">
        <el-form-item label="供应商" prop="supplier_id">
          <el-select v-model="paymentForm.supplier_id" placeholder="请选择供应商" filterable style="width: 100%">
            <el-option v-for="sup in suppliers" :key="sup.id" :label="sup.name" :value="sup.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="入库金额" prop="inbound_amount">
          <el-input-number v-model="paymentForm.inbound_amount" :min="0" :precision="2" style="width: 100%" />
        </el-form-item>
        ...
        <el-form-item label="公司账户" prop="company_account_id">
          <el-select v-model="paymentForm.company_account_id" placeholder="请选择付款账户" style="width: 100%">
            <el-option v-for="acc in companyAccounts" :key="acc.id" :label="`${acc.account_name} (${acc.account_number})`" :value="acc.id" />
          </el-select>
        </el-form-item>
...
async function fetchPayments() {
  ...
  if (error) ElMessage.error('加载付款记录失败: ' + error.message)
  else {
...
async function fetchSuppliersAndAccounts() {
  ...
  if (supRes.error) ElMessage.error('加载供应商失败')
  else suppliers.value = supRes.data || []
  if (accRes.error) ElMessage.error('加载公司账户失败')
...
async function savePayment() {
  ...
  let error
  if (paymentForm.id) {
    const { error: updateErr } = await supabase.from('payment_orders').update(dataToSave).eq('id', paymentForm.id)
    error = updateErr
  } else {
    const { error: insertErr } = await supabase.from('payment_orders').insert([dataToSave])
    error = insertErr
  }
...
function openConfirmDialog(row) {
  ElMessageBox.confirm(`确认已向 ${row.supplier_name} 支付 ${formatMoney(row.paid_amount)} 元？`, '确认打款', {
    type: 'info',
    ...
    const { error } = await supabase
      ...
      .eq('id', row.id)
    if (error) ElMessage.error('打款确认失败: ' + error.message)
    else {
